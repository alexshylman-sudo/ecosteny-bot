...
async def perform_text_calc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_payload = (
        ...
        f"{extra_sizes}"
        "Задача:\n"
        "1) Для КАЖДОЙ выбранной категории материалов (стеновые панели, реечные панели, 3D панели) "
        "рассчитать примерное количество панелей и ориентировочную стоимость.\n"
        "2) Обязательно выводи категории отдельно: «Стеновые панели», «Реечные панели», «3D панели».\n"
    ...
    await update.effective_message.reply_text(
        "Если хотите сделать ещё один расчёт или задать другой вопрос — выберите вариант ниже:",
        reply_markup=build_main_menu_keyboard(),
    )

    # Сброс состояния расчёта
    context.chat_data["main_mode"] = None
    context.chat_data["calc_phase"] = None
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Назад в меню
    if action == "ui" and len(parts) >= 2 and parts[1] == "back_main":
        context.chat_data["main_mode"] = None
        context.chat_data["calc_phase"] = None
        context.chat_data["calc_items"] = []
        context.chat_data["materials_locked"] = False
        ...
        context.chat_data["height_mode"] = None
        context.chat_data["await_custom_name_index"] = None
        await query.edit_message_text("Чем могу помочь?", reply_markup=build_main_menu_keyboard())
        return
    ...
    if action == "info_cat" and len(parts) >= 2:
        ...
        if cat == "walls":
            ...
            await query.message.reply_text("Если хотите, опишите стену — помогу прикинуть расход.")
            return
        ...
        if cat == "slats":
            ...
            await query.edit_message_text(text, parse_mode="HTML")
            await query.message.reply_text("Напишите ширину и высоту стены — прикину расход реек.")
            return
        ...
        if cat == "3d":
            ...
            await query.edit_message_text(text, parse_mode="HTML")
            await query.message.reply_text("Опишите вашу стену — подскажу, сколько 3D панелей потребуется.")
            return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 0. Название/артикул для последнего материала (ТОЛЬКО на этапе выбора материалов)
    custom_index = context.chat_data.get("await_custom_name_index")
    if (
        custom_index is not None
        and main_mode == "calc"
        and calc_phase == "select_materials"
    ):
        items = context.chat_data.get("calc_items", [])
